  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        isScrolled || mobileMenuOpen ? "bg-white shadow-md py-2" : "bg-gradient-to-b from-black/50 to-transparent py-4"
      }`}
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center">
            <Image
              src={isScrolled || mobileMenuOpen ? "/images/logo-color.png" : "/images/logo-white.png"}
              ...
              <div key={link.label} className="relative group">
                <Link
                  ...
                  className={`font-medium transition-colors ${
                    isScrolled ? "text-brand-blue hover:text-brand-red" : "text-white hover:text-white/80"
                  } ${pathname === link.href ? "font-bold text-brand-red" : ""}`}
                >
                ...
                {link.children && (
                  <div className="absolute left-0 mt-4 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform-gpu scale-95 group-hover:scale-100 origin-top-left">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            <a
              ...
              className={`flex items-center text-sm font-medium transition-colors ${
                isScrolled ? "text-gray-800 hover:text-brand-red" : "text-white hover:text-white/80"
              }`}
            >
            ...
            <div className="container mx-auto px-4 pt-4 pb-8">
              <div className="flex flex-col space-y-2">
                    ...
                    {link.children ? (
                      <div>
                        <button
                          ...
                          className="w-full flex items-center justify-between py-3 text-lg font-medium text-brand-blue hover:text-brand-red"
                        >
                          {link.label}
                          <motion.div
                        ...
                        <AnimatePresence>
                          {expandedMenus.includes(link.label) && (
                            ...
                            >
                              <div className="pt-2 pb-3 ml-4 space-y-2 border-l-2 border-brand-blue/20 pl-4">
                                {link.children.map((child) => (
                                  <Link
                                    key={child.label}
                                    href={child.href}
                                    className="block py-1 text-gray-600 hover:text-brand-blue"
                                  >
                                    {child.label}
                                  </Link>
                                ))}
                      ...
                      <Link
                        href={link.href}
                        className="block py-3 text-lg font-medium text-brand-blue hover:text-brand-red"
                      >
                        {link.label}
                      </Link>
                    )}
                ...
                <div className="pt-6 space-y-4">
                  <a
                    href="tel:+19529004222"
                    className="flex items-center justify-center w-full py-3 bg-gray-100 rounded-lg text-gray-800 font-medium hover:bg-gray-200 transition-colors"
                  >
